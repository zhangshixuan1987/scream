...
public:
  ...
  //   - the run method time-advances the process by one time step. The two timesteps
  //     are, respectively, the timestep of the whole atmosphere, and the timestep
  //     that this process should advance by.
  ...
  void run (const int dt_atm, const int dt_process);
...
protected:
  ...
  virtual void run_impl(const int dt_atm, const int dt_process) = 0;
